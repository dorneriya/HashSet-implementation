...
public class BasicAPITests {
	...
	/**
	 * Determines if these hashset.tests run on all the words in the files data1.txt, data2.txt or part of
	 * them
	 */
	...
	private static CollectionFacadeSet javaTreeSet =
            new CollectionFacadeSet(new TreeSet<String>());

	private static CollectionFacadeSet javaLinkedList = new CollectionFacadeSet(
			new LinkedList<String>());

	private static CollectionFacadeSet javaHashSet =
	...
	public static void initWords() {
		if (allWords) {
            ...
            for (int i = 0; i < partOfWordsLength; i++) {
                wordsData1[i] = allWords1[i];
            }
            for (int i = 0; i < partOfWordsLength; i++) {
                wordsData2[i] = allWords2[i];
            }
		}
	}
	 ...
	 * @param obj - object to test. hashset.OpenHashSet/hashset.ClosedHashSet/TreeSet/ect.
	...
	private void testOnData(SimpleSet obj, String[] words) {
        ...
        for (int i = 0; i < words.length; i++) {
            Assert.assertTrue(obj.contains(words[i]));
        }
        ...
        for (int i = 0; i < 100; i++) {
            Assert.assertFalse(obj.add(words[i])); // no duplicates
        }
        for (int i = 0; i < 100; i++) {
            boolean b = obj.delete(words[i]);
            Assert.assertTrue(b);
        }
        for (int i = 0; i < 100; i++) {
            boolean b = obj.delete(words[i]);
            Assert.assertFalse(b); //deleted
        }
        for (int i = 0; i < 100; i++) {
            Assert.assertFalse(obj.contains(words[i])); //deleted
        }
